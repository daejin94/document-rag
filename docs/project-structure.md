...
│       │   ├── admin/        # SUPER_ADMIN 관리자 API, 사용량 집계
...
│       │   ├── project/      # 프로젝트, 멤버, 접근 제어 가드
│       │   ├── usage/        # 토큰 사용량 기록(token_usages)
│       │   └── user/         # 사용자 Entity/Repository(전역 역할 포함)
│       └── resources/
│           ├── application.yml
...
│   └── src/
│       ├── api.ts            # Backend API 호출 래퍼(401 처리 포함)
│       ├── App.tsx           # 인증/워크스페이스/관리자 화면 조율
│       ├── components/       # 화면 컴포넌트
│       │   └── admin/        # SUPER_ADMIN 관리자 화면
...
- `project`: 프로젝트/멤버 CRUD와 접근 제어 가드(`requireMember`/`requireAdmin`)를 담당한다.
- `document`: 문서 업로드, 파일 저장, 텍스트 추출, chunk 생성, embedding 저장, vector search 쿼리를 담당한다. 작업은 프로젝트 멤버십으로 제한된다.
- `chat`: 질문 처리, 채팅 세션/메시지 저장, 답변 출처 저장과 조회를 담당한다. 작업은 프로젝트 스코프다.
- `llm`: OpenAI API 호출을 감싼다. 모델명은 환경변수를 우선 사용한다.
- `usage`: OpenAI 호출의 토큰 사용량을 `token_usages`에 기록한다.
- `admin`: `SUPER_ADMIN` 전용 사용자/프로젝트/사용량 조회 API를 담당한다.
- `common`: API 예외와 공통 에러 응답을 담당한다.
- `user`: 사용자 데이터 접근과 전역 역할(`USER`/`SUPER_ADMIN`)을 담당한다.

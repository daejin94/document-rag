...
3. 질문으로 채팅 세션을 생성한다.
4. 사용자 메시지를 저장한다.
5. 질문 embedding을 생성한다.
6. `document_chunks`에서 유사 chunk를 검색한다.
7. 검색 결과가 없거나 최상위 similarity가 threshold보다 낮으면 고정 답변을 반환한다.
8. 검색 결과가 있으면 prompt를 구성해 OpenAI Chat API를 호출한다.
9. assistant 메시지를 저장한다.
10. 답변 출처로 사용된 chunk를 `answer_sources`에 저장한다.
11. 답변, 출처, 모델명, usage를 반환한다.
...
검색 결과가 없거나 최상위 similarity가 threshold보다 낮으면 아래 답변을 반환한다.

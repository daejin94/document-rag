...
1. 사용자가 `/api/projects/{projectId}/documents`에 `title`, `file`을 multipart로 전송한다.
...
1. 사용자가 `/api/projects/{projectId}/chat/query`에 `question`, `documentIds`, 선택적으로 `sessionId`를 전송한다.
2. 서버는 `requireMember`로 프로젝트 권한을 확인하고, `documentIds`가 해당 프로젝트 소속인지 검증한다.
...
6. 검색은 요청 프로젝트의 `COMPLETED` 상태 문서로 제한된다.
...
- 접근 권한이 없는(프로젝트 멤버가 아닌) 문서 chunk가 검색 결과에 포함되면 안 된다.

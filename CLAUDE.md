...
- **`document`** — 업로드 파이프라인은 **`DocumentService.upload` 안에서 동기 실행**된다: 파일 저장 → `TextExtractor`(UTF-8 후 MS949 fallback; PDF는 PDFBox, 텍스트 추출 가능 문서만) → `DocumentChunker`(크기/overlap은 config) → 각 chunk embedding → 저장. 상태 전이는 `PROCESSING → COMPLETED | FAILED`이며, `COMPLETED` 문서만 검색 대상이 된다.

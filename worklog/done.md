...
- 검증: backend `./gradlew build`(테스트 포함) 통과, frontend `npm run build`(tsc+vite) 통과. E2E도 실제 스택(Docker Postgres + bootRun + Vite)에서 브라우저로 확인 완료 — TXT 업로드 시 모달 즉시 닫힘 → 사이드바 `PROCESSING` 표시 → 폴링만으로 `COMPLETED` 자동 전환 → 해당 문서 키워드 질문에 chunk 검색(유사도 34.5%)·출처 반환 정상.
